...
def fix_capitalization(html_content):
    """Fix capitalization in HTML content to sentence case for headers"""
    # Fix h1-h6 headers
    def capitalize_header(match):
        tag = match.group(1)
        content = match.group(2)
        # Convert to sentence case: capitalize first letter and after punctuation
        sentences = re.split(r'([.!?]\s*)', content)
        result = []
        for i, sentence in enumerate(sentences):
            if i % 2 == 0:  # actual text
        ...
        return f'<{tag}>{ "".join(result) }</{tag}>'
    
    # Apply to all headers
    html_content = re.sub(r'<(h[1-6])>([^<]+)</\1>', capitalize_header, html_content, flags=re.IGNORECASE)
...
def markdown_to_html(md_content, article_num):
    ...
    html_content = fix_capitalization(html_content)
...
def fix_html_capitalization():
    """Fix capitalization in existing HTML article files"""
    ...
    for i in range(1, 8):
        ...
        if html_file.exists():
            print(f"Fixing capitalization in article-{i}.html...")
            with open(html_file, 'r', encoding='utf-8') as f:
                html_content = f.read()
            
            fixed_content = fix_capitalization(html_content)
